...
video_path = "target1.mp4"  # Update path to the uploaded file
        ...
        if radius > 15:  # Filter small contours to avoid noise
            ...
            if len(ball) > 1:
                for i in range(1, len(ball)):
                    cv2.line(frame, ball[i-1], ball[i], (0, 255, 0), 1)

            if len(ball) >= 360:
                    ball = ball[-360:]  # Keep only the last 360 points
                    trajectory_cleared = True  

        if trajectory_cleared:
             ball = []
             trajectory_cleared = False                  
